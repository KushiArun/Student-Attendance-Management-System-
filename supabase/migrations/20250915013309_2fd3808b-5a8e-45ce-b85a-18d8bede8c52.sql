...
FOR SELECT USING (public.get_current_user_role() IN ('ADMIN', 'HOD', 'PRINCIPAL'));

CREATE POLICY "Admins can insert classes" ON public.classes
FOR INSERT WITH CHECK (public.get_current_user_role() IN ('ADMIN', 'HOD', 'PRINCIPAL'));

CREATE POLICY "Admins can update classes" ON public.classes
FOR UPDATE USING (public.get_current_user_role() IN ('ADMIN', 'HOD', 'PRINCIPAL'));

-- RLS Policies for students
CREATE POLICY "Admins can view all students" ON public.students
FOR SELECT USING (public.get_current_user_role() IN ('ADMIN', 'HOD', 'PRINCIPAL', 'TEACHER'));
...
CREATE POLICY "All authenticated users can view student_classes" ON public.student_classes
FOR SELECT TO authenticated USING (true);

-- RLS Policies for timetable
CREATE POLICY "All authenticated users can view timetable" ON public.timetable
FOR SELECT TO authenticated USING (true);

CREATE POLICY "Admins can insert timetable" ON public.timetable
FOR INSERT WITH CHECK (public.get_current_user_role() IN ('ADMIN', 'HOD', 'PRINCIPAL'));
...
FOR SELECT USING (
  ...
  ) OR public.get_current_user_role() IN ('ADMIN', 'HOD', 'PRINCIPAL')
);
